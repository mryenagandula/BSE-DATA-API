from corporate_announcements import loadBSEAnnouncements,doDataSummary
import datetime
import os
import threading
import logging
...
def main():
    ...
    pauseDownloadReports = input['pauseDownloadReports'];
    pdfFilesListSummary: bool = input['pdfFilesListSummary'];
    if selectInputOption == ALL_ANNOUNCEMENTS:
        ...
        attachmentFilePath = utils.getAutoAttachmentFileName(announcementsFilePath);
        loadBSEAnnouncements(pauseDownloadReports,attachmentFilePath,pdfFilesListSummary,fileName + ".csv");
        end_time = time.time();
        logging.info(f"Ended process time end_time = {end_time}");
        elapsed_time = end_time - start_time;
        elapsed_minutes = elapsed_time / 60
        logging.info(f"Elapsed time: {elapsed_minutes:.2f} minutes")
    if selectInputOption == SUMMARY_LATER:
        strTotal = selectInputOption;
        logFileName = utils.getAutoGeneratedLogFileName(strTotal);
        logConfig(logFileName);
        logging.info(f"Started process time start_time = {start_time}");
